...
export const createUser = async (payload: any) => {
  const supabase = await supabaseServer()
    .from("users")
    .insert({ fullname: payload?.name, email: payload?.email });
...
export const createCustomPath = async (
  path: string,
  localLink: string
): Promise<CreateCustomPathResponse> => {
  ...
  if (session?.user) {
    ...
    const create = await supabase
      .from("links")
      .insert({ path, email: session.user?.email, link: localLink });
